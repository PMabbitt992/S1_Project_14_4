...
function defineDataArray() {
      ...
      var cellContent = tableData.sort([dataSort2D()]);
...
function writeTableData() {
      ...
      for (var b = 0; b < /*for each table row*/ .length; b++) {
            var tRow = document.createElement("tr");
            for (var c = 0; c < /*for each table data in each row*/ .length; c++) {
                  var tCell = document.createElement("td");
                  var contentText = cellContent /*the value spit out by tableData (sorted)*/ ;
            }
      }
      tCell.appendChild(contentText);
